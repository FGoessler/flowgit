...
describe('gf create', () => {
  ...
  describe('with staged changes', () => {
    ...
    it('handles special characters in commit message', async () => {
      ...
      expect(testRepo.currentBranch()).toBe('fix-bug-with-useremailcom-validation');
    });
  });
  ...
  describe('edge cases', () => {
    it('handles very long file paths', async () => {
      ...
      const longPath = 'very/deeply/nested/directory/structure/that/goes/on/for/a/while';
    ...
    it('handles unicode characters in commit messages', async () => {
      ...
      expect(testRepo.currentBranch()).toBe('add-feature-with-mojis-and-spcial-hars');
      expect(testRepo.lastCommitMessage()).toBe('Add feature 🚀 with émojis and spëcial çhars');
    });
    ...
    it('handles dirty working directory with untracked files', async () => {
      ...
      // Act
      await runCommand(['create'], testRepo, {
        prompts: { message: 'Add tracked file' },
      });
