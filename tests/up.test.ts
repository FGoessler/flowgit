...
describe('gf up', () => {
  ...
  it('shows picker when multiple children exist', async () => {
    ...
    testRepo.git('config flowgit.tracked "parent-branch,child1-branch,child2-branch"');
    testRepo.git('config flowgit.branch.parent-branch.parent "main"');
    ...
    await runCommand(['up'], testRepo, { prompts: { branch: 'child1-branch' } });
  ...
  it('shows error when no children exist', async () => {
    ...
    // Act & Assert
    await expect(runCommand(['up'], testRepo)).rejects.toThrow('process.exit(1)');
  });
