...
import * as claude from "../src/lib/claude";
...
describe("fgt submit", () => {
  ...
  describe("single branch submit", () => {
    ...
    it("creates PR when none exists", async () => {
      ...
      mockExecutor
        .onCommand(/gh pr create/)
        .returns("https://github.com/test/repo/pull/123");
    ...
    it("updates existing PR by pushing", async () => {
      ...
      // Act - confirm defaults to false, so description is not updated
      await runCommand(["submit", "--current"], testRepo);

      // Assert - should NOT create a new PR or update description
      const calls = mockExecutor.getCalls();
    ...
    it("updates PR description when user opts in", async () => {
      // Arrange
      const remote = testRepo.createRemote();

      mockClaude.isClaudeInstalled.mockReturnValue(true);
      mockClaude.generatePRDescription.mockReturnValue("Generated description");
      ...
      mockExecutor.onCommand(/gh pr edit/).returns("");

      // Act - user opts in to updating description
      await runCommand(["submit", "--current"], testRepo, {
        prompts: { confirmed: true },
      });

      // Assert - should call gh pr edit with generated description
      const calls = mockExecutor.getCalls();
      expect(calls.some((c) => c.includes("gh pr create"))).toBe(false);
      expect(calls.some((c) => c.includes("gh pr edit 456"))).toBe(true);
      expect(mockClaude.generatePRDescription).toHaveBeenCalled();
    });

    it("skips description update when Claude is not installed", async () => {
      // Arrange
      const remote = testRepo.createRemote();

      mockClaude.isClaudeInstalled.mockReturnValue(false);
      ...
      // Act - user opts in but Claude is not available
      await runCommand(["submit", "--current"], testRepo, {
        prompts: { confirmed: true },
      });

      // Assert - should not call gh pr edit
      const calls = mockExecutor.getCalls();
      expect(calls.some((c) => c.includes("gh pr edit"))).toBe(false);
    ...
    it("extracts Linear ticket IDs from commit message", async () => {
      ...
      mockExecutor
        .onCommand(/gh pr create/)
        .returns("https://github.com/test/repo/pull/789");
      mockExecutor.onCommand(/gh pr view.*--json/).returns(
        JSON.stringify({
          number: 789,
    ...
    it("sets base branch to parent branch", async () => {
      ...
      mockExecutor
        .onCommand(/gh pr create/)
        .returns("https://github.com/test/repo/pull/100");
      mockExecutor.onCommand(/gh pr view.*--json/).returns(
        JSON.stringify({
          number: 100,
          title: "Test",
      ...
      const createCall = calls.find(
        (c) => c.includes("gh pr create") && c.includes("child"),
      );
  ...
  describe("stack submit", () => {
    beforeEach(() => {
      ...
      testRepo.git(
        'config flowgit.tracked "api-layer,business-logic,ui-layer"',
      );
    });
    ...
    it("creates PRs with correct parent relationships", async () => {
      ...
      mockExecutor
        .onCommand(/gh pr create/)
        .returns("https://github.com/test/repo/pull/1");
      mockExecutor.onCommand(/gh pr view.*--json/).returns(
        JSON.stringify({
          number: 1,
          title: "Test",
      ...
      const businessCall = createCalls.find((c) =>
        c.includes("Add business logic"),
      );
    ...
    it("only submits current branch with --current flag", async () => {
      ...
      mockExecutor
        .onCommand(/gh pr create/)
        .returns("https://github.com/test/repo/pull/3");
      mockExecutor.onCommand(/gh pr view.*--json/).returns(
        JSON.stringify({
          number: 3,
          title: "Add UI layer",
  ...
  describe("remote operations", () => {
    ...
    it("pushes with -u when no remote exists", async () => {
      ...
      const calls = mockExecutor.getCalls();
      expect(
        calls.some((c) => c.includes("git push -u origin test-branch")),
      ).toBe(true);
    });
    ...
    it("regular push when remote tracking exists", async () => {
      ...
      const pushCalls = calls.filter(
        (c) => c.includes("git push") && !c.includes("git push -u"),
      );
    ...
    it("force push when remote is ahead", async () => {
      ...
      expect(
        calls.some(
          (c) => c.includes("git push") && c.includes("--force-with-lease"),
        ),
      ).toBe(true);
    });
